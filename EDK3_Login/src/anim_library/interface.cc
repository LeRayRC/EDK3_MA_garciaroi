...
void UpdateFramerateWindow() {
  ...
  game->fps_timer += game->dt;
  if (game->fps_timer >= game->refresh_fps_cooldown) {
    game->fps_timer = 0.0f;
    game->framerate = ImGui::GetIO().Framerate;
  }
}
...
void SetFlags(ImGuiWindow *window) {
  ...
  if (window->flags.no_nav)
    window->window_flags |= ImGuiWindowFlags_NoNav;
  if (window->flags.no_background)
    window->window_flags |= ImGuiWindowFlags_NoBackground;
  if (window->flags.no_bring_to_front)
    window->window_flags |= ImGuiWindowFlags_NoBringToFrontOnFocus;
  if (window->flags.unsaved_document)
    window->window_flags |= ImGuiWindowFlags_UnsavedDocument;
}

void WindowSettings() {
  Game *game = Game::getInstance();
  game->settings_window.flags.no_background = true;
  game->settings_window.flags.no_resize = false;
  SetFlags(&game->settings_window);
  ImGui::SetNextWindowPos(ImVec2(0, 0), ImGuiCond_FirstUseEver);
  // ImGui::Begin("Settings", NULL, game->settings_window.window_flags);
  if (ImGui::BeginMainMenuBar()) {
    if (ImGui::BeginMenu("Windows")) {
      if (ImGui::BeginMenu("Settings")) {
        ImGui::Checkbox("No Titlebar", &game->settings_window.flags.no_titlebar);
        ImGui::EndMenu();
      }
      if (ImGui::BeginMenu("Entities")) {
        ImGui::Checkbox("Opened", &game->entities_window.popen);
        ImGui::Checkbox("No Titlebar", &game->entities_window.flags.no_titlebar);
        ImGui::Checkbox("No Resize", &game->entities_window.flags.no_resize);
        ImGui::Checkbox("No Move", &game->entities_window.flags.no_move);
        ImGui::Checkbox("No Background", &game->entities_window.flags.no_background);
        ImGui::EndMenu();
      }
      if (ImGui::BeginMenu("Animation Configs")) {
        ImGui::Checkbox("Opened", &game->animationconfigs_window.popen);
        ImGui::Checkbox("No Titlebar", &game->animationconfigs_window.flags.no_titlebar);
        ImGui::Checkbox("No Resize", &game->animationconfigs_window.flags.no_resize);
        ImGui::Checkbox("No Move", &game->animationconfigs_window.flags.no_move);
        ImGui::Checkbox("No Background", &game->animationconfigs_window.flags.no_background);
        ImGui::EndMenu();
      }
      ImGui::EndMenu();
    }
...
void InitDefaultWindowFlags(ImGuiWindow *window, bool default_opened) {
  ...
  window->flags.no_nav = false;
  window->flags.no_background = false;
  window->flags.no_bring_to_front = false;
  window->flags.unsaved_document = false;
...
void WindowMenu(ImGuiWindow *window) {
  if (ImGui::BeginMenuBar()) {
    if (ImGui::BeginMenu("Options")) {
      ...
      ImGui::Checkbox("No Background", &window->flags.no_background);
      ImGui::EndMenu();
    }
...
void WindowsController() {
  Game *game = Game::getInstance();
  if (game->settings_window.popen) {
    WindowSettings();
  }
  if (game->entities_window.popen) {
